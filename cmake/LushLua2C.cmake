...
function(_LuaScript name)
    ...
    set(_GET_FILE_SIZE)
    if (APPLE)
        set(_GET_FILE_SIZE `stat -f%z ${name}.out`)
    else()
        set(_GET_FILE_SIZE `stat -c%s ${name}.out`)
    endif()

    # convert to c file
    add_custom_command(
            COMMAND echo const char ${LUA2C_VARIABLE}[] = { `cat ${${name}_C_FILE}.hex` 0x00 } \"\\;\" const unsigned long long ${LUA2C_VARIABLE}_size = ${_GET_FILE_SIZE} \"\\;\" > ${${name}_C_FILE}
            OUTPUT ${${name}_C_FILE}
            WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
